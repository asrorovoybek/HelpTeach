...
def log_runtime_diagnostics():
    logger.info("Python process boshlandi")
    logger.info("PORT=%s", os.getenv("PORT", "8080"))
    logger.info("BOT_TOKEN mavjud=%s", bool(BOT_TOKEN))
    logger.info("SUPABASE_URL mavjud=%s", bool(os.getenv("SUPABASE_URL")))
    logger.info("SUPABASE_KEY mavjud=%s", bool(os.getenv("SUPABASE_KEY")))
    logger.info("WEATHER_API_KEY mavjud=%s", bool(os.getenv("WEATHER_API_KEY")))


def log_dns_status():
    try:
        ip = socket.gethostbyname("api.telegram.org")
        logger.info("api.telegram.org resolved -> %s", ip)
    except Exception as e:
        logger.exception("DNS resolve xatosi: %s", e)


async def safe_delete_webhook(max_attempts: int = 5):
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            logger.error("delete_webhook xato (attempt %s/%s): %s", attempt, max_attempts, e)
            if attempt < max_attempts:
                await asyncio.sleep(min(5 * attempt, 20))
    return False


async def safe_set_commands():
    try:
        await set_bot_commands()
        logger.info("Bot commandlar o'rnatildi")
    except Exception as e:
        logger.error("set_bot_commands xato: %s", e)
...
async def polling_forever():
    ...
    logger.info("Allowed updates: %s", allowed_updates)

    while True:
        try:
            logger.info("Polling boshlanmoqda...")
            await dp.start_polling(bot, allowed_updates=allowed_updates)
            logger.warning("Polling to'xtadi. 3 soniyadan keyin qayta urinish bo'ladi.")
            await asyncio.sleep(3)
        except TelegramRetryAfter as e:
            wait_time = int(getattr(e, "retry_after", 5) or 5)
            logger.warning("TelegramRetryAfter: %s soniya kutilyapti", wait_time)
            await asyncio.sleep(wait_time)
        except TelegramConflictError as e:
            logger.error("TelegramConflictError: %s", e)
            logger.error("Bot boshqa joyda ham ishlayotgan bo'lishi mumkin. 10 soniyadan keyin qayta urinish.")
