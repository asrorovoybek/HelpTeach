...
# Eslatma oynasi ichida takroriy yuborishni oldini olish uchun xotira cache
_recent_lesson_notifications: Dict[Tuple[int, int, str, str], datetime] = {}


def _prune_recent_notifications(now: datetime) -> None:
    expired = [key for key, ts in _recent_lesson_notifications.items() if (now - ts) > timedelta(hours=6)]
    for key in expired:
        _recent_lesson_notifications.pop(key, None)


def _should_send_lesson_notice(uid: int, lesson_id: int, notice_type: str, lesson_start_dt: datetime, now: datetime) -> bool:
    minutes_left = (lesson_start_dt - now).total_seconds() / 60

    if notice_type == "before_30":
        in_window = 25 <= minutes_left <= 30
    elif notice_type == "before_10":
        in_window = 5 <= minutes_left <= 10
    elif notice_type == "on_time":
        in_window = -1 <= minutes_left <= 1
    else:
        in_window = False
    ...
    dedupe_key = (uid, lesson_id, notice_type, now.strftime("%Y-%m-%d"))
    if dedupe_key in _recent_lesson_notifications:
        return False

    _recent_lesson_notifications[dedupe_key] = now
    return True
...
async def check_upcoming_lessons(bot):
    try:
        ...
        _prune_recent_notifications(now)
        ...
        logger.info("check_upcoming_lessons ishladi: %s | lessons=%s", now.isoformat(), len(all_lessons))
        ...
        for lesson in all_lessons:
            ...
            try:
                lesson_start_dt = datetime.strptime(
                    f"{now.strftime('%Y-%m-%d')} {lesson_start}",
                    "%Y-%m-%d %H:%M",
                ).replace(tzinfo=TZ)
            except Exception:
                logger.warning("Noto'g'ri lesson time, lesson=%s", lesson)
                continue
            ...
            if settings.get("notify_before_30") and _should_send_lesson_notice(uid, lesson_id, "before_30", lesson_start_dt, now):
            ...
            if settings.get("notify_before_10") and _should_send_lesson_notice(uid, lesson_id, "before_10", lesson_start_dt, now):
                await safe_send_message(
                    bot,
                    uid,
                    (
            ...
            if settings.get("notify_on_time") and _should_send_lesson_notice(uid, lesson_id, "on_time", lesson_start_dt, now):
                await safe_send_message(
                    bot,
                    uid,
                    (
        ...
        logger.exception(f"check_upcoming_lessons xatosi: {e}")
...
def setup_scheduler(bot):
    ...
    common = {"args": [bot], "max_instances": 1, "coalesce": True, "misfire_grace_time": 120, "replace_existing": True}
    scheduler.add_job(check_upcoming_lessons, "interval", minutes=1, id="check_upcoming_lessons", **common)
    scheduler.add_job(check_custom_reminders, "interval", minutes=1, id="check_custom_reminders", **common)
    scheduler.add_job(send_morning_message, "interval", minutes=1, id="send_morning_message", **common)
    scheduler.add_job(send_evening_summary, "interval", minutes=1, id="send_evening_summary", **common)

    scheduler.start()
    logger.info("✅ Scheduler ishga tushdi! Jobs: %s", [job.id for job in scheduler.get_jobs()])
    return scheduler
